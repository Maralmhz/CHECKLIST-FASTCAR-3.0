// firebase_app.js - VERSÃO ORGANIZADA POR PASTAS - FASTCAR
...
async function initFirebase() {
    ...
    try {
        ...
        console.log('✅ Firebase inicializado com sucesso! [FastCar]');
...
function getOficinaId() {
    return window.OFICINA_CONFIG?.oficina_id || 'fastcar';
}
...
export async function buscarChecklistsNuvem() {
    try {
        console.log('⏳ Buscando TODOS os checklists (pode demorar)...');
...
export async function migrarChecklistsParaPastas() {
    try {
        console.log('🔄 Iniciando migração para estrutura organizada [FastCar]...');
...
export async function verificarConexaoFirebase() {
    try {
        ...
        return {
            status: 'conectado',
            mensagem: 'Firebase conectado com sucesso! [FastCar]',
            oficina: oficinaId,
...
export async function exibirEstatisticas() {
    try {
        console.log('📊 === ESTATÍSTICAS DO FIREBASE [FASTCAR] ===');
...
if (typeof window !== 'undefined') {
    ...
    console.log('🔧 === COMANDOS FIREBASE DISPONÍVEIS [FASTCAR] ===');
